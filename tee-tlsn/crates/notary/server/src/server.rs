...
pub async fn run_server(config: &NotaryServerProperties) -> Result<(), NotaryServerError> {
    ...
    let provider_processor = Processor::new(config.provider.json_path.clone(), config.provider.schema_url.clone()).await.map_err(|e| eyre!("Failed to initialize provider: {}", e))?;
